...
//declarando vetores e variáveis
		...
		double preco[] = { 2000.15, 4000.87, 899.99, 3000.50, 900.45, 120.99, 140.99, 129.01, 200.99, 140.99 },
				total = 0.0, parcelas = 0.0, imposto, totalItem[] = new double[10];
		boolean continua = true, prosseguir = true;
		char pagamento = '0';
		Scanner read = new Scanner(System.in);

		System.out.print("Bem Vindo a 91Zac_Games, A maior loja de Games da internet!\n");
		...
		while (prosseguir) {
			...
			while (continua) {
				System.out.print("Digite o número correspondente ao produto que deseja comprar ");
				System.out.println("\n-----------------------------------------------------------------------------");
				codigo = read.nextInt();
			...
			System.out.println("-----------------------------------------------------------------------------");
			System.out.println("Qtd:\tProduto:\tValor Unitário:\tTotal do Item ");
			for (int i = 0; i < 10; i++) {
				if (compras[i] != 0) {
			...
			seguir = read.next();
			prosseguir = (seguir.toUpperCase().charAt(0) == 'N') ? false : true;

		}
		System.out.println("Nós da 91Zac_Games agradecemos a Preferência.");
	}
}
